...
import { Home, Target, ShoppingBag, Calendar, User } from 'lucide-react'
        ...
        {tabs.map(({ id, label, Icon }) => {
          ...
          return (
            <motion.button
              key={id}
              onClick={() => {
                if (isShop && active) { setIsOpen(true); return }
                navigate(id)
              }}
            ...
            >
              ...
              <div className="relative">
                ...
                />
                {isShop && count > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-lime text-[#0D0D0D] text-[9px] font-bold flex items-center justify-center">
                    {count > 9 ? '9+' : count}
                  </span>
                )}
              </div>
